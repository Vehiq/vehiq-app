# UptimeRobot — Konfiguracja Monitoringu Sharago

Krótki przewodnik konfiguracji bezpłatnego monitoringu uptime dla backendu Sharago wdrożonego na Render.com. UptimeRobot regularnie pinguje endpoint zdrowia, co dodatkowo zapobiega "uśpieniu" instancji Free na Render (cold start co 15 min bezczynności).

---

## 1. Wymagania wstępne

- Wdrożony backend Sharago (Render lub inna platforma)
...
|------|---------|
| **Monitor Type** | `HTTP(s)` |
| **Friendly Name** | `Sharago Backend Health` |
...
Dla pełnej obserwowalności Sharago zalecamy:

| Nazwa | URL | Po co? |
|-------|-----|--------|
| `Sharago Frontend` | `https://vehiq.vercel.app` | Sprawdza, czy SPA się ładuje |
| `Sharago AI Endpoint` | `https://...onrender.com/api/ai/status` | Sprawdza dostępność klucza Anthropic |
| `Sharago Search` | `https://...onrender.com/api/search?q=test` | Sprawdza pełen flow MongoDB + API |
...
2. Nadaj nazwę (np. `Sharago Status`) i custom URL: `vehiq.statuspage.uptimerobot.com`
3. Wybierz monitory do publicznego pokazania
4. Skopiuj link i osadź np. w stopce `sharago.pl/status`
...
Gotowe. Masz teraz darmowy 24/7 monitoring produkcji Sharago. 🚀
